...
export function renameCitationKeyUsages(markdown: string, originalKey: string, nextKey: string): string {
  ...
  for (const bracket of markdown.matchAll(/\[[^\]]*@[^\]]*]/g)) {
    const bracketOffset = bracket.index ?? 0;
    if (isOffsetInsideRanges(bracketOffset, ignoredRanges)) continue;
    for (const keyMatch of bracket[0].matchAll(/@([A-Za-z0-9_][A-Za-z0-9_:.#$%&+\-?<>~/]*)/g)) {
      if (keyMatch[1] !== normalizedOriginal) continue;
      const from = bracketOffset + (keyMatch.index ?? 0);
      replacements.push({
        from,
...
function extractUsedCitationKeys(markdown: string): string[] {
  ...
  for (const bracket of markdown.matchAll(/\[[^\]]*@[^\]]*]/g)) {
    if (isOffsetInsideRanges(bracket.index ?? 0, ignoredRanges)) continue;
    for (const keyMatch of bracket[0].matchAll(/@([A-Za-z0-9_][A-Za-z0-9_:.#$%&+\-?<>~/]*)/g)) {
