...
import { appCss } from '../styles/appCssBundle';
...
import { createExportArtifactIssue, exportIssueAttributes } from '../export/exportArtifactIssues';
...
async function renderMarkdownHtml(markdown: string, options: Required<RenderMarkdownOptions>): Promise<string> {
  ...
  const directivePass = options.renderDirectives
    ? await replaceDirectiveBlocks(pageBreakPass.markdown, {
      citationEntries: options.citationEntries,
      renderMarkdownHtml: (bodyMarkdown) => renderMarkdownHtml(bodyMarkdown, options),
    })
...
function missingImageDataUri(altText: string, markdownPath: string): string {
  ...
  const issue = createExportArtifactIssue('missing-image', markdownPath);
  const svg = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="220" viewBox="0 0 1200 220" role="img" ${exportIssueAttributes(issue)}>`,
